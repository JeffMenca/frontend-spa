...
import type { CongressesByInstitutionReportData } from "@/lib/validators/reports";
...
function buildHtml(data: CongressesByInstitutionReportData): string {
  const rows = data.items
    .map(
      (c) =>
        `<tr>
          <td>${esc(c.institutionName)}</td>
          <td>${esc(c.congressName)}</td>
          <td>${esc(formatDateLocal(c.startDate))}</td>
          <td>${esc(formatDateLocal(c.endDate))}</td>
          <td>${esc(c.location)}</td>
          <td style="text-align:right;font-weight:500">Q ${c.price.toFixed(2)}</td>
        </tr>`,
    )
    .join("");
  ...
  return buildReportPage(
    ...
    "Reporte de congresos agrupados por institucion",
    data.totalItems,
    ["Institucion", "Congreso", "Inicio", "Fin", "Lugar", "Precio"],
    rows,
  );
}
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const data = await activeConference.getCongressesByInstitutionReport(token, params);
