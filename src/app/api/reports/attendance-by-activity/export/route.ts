...
import type { AttendanceByActivityReportData } from "@/lib/validators/reports";
...
function buildHtml(data: AttendanceByActivityReportData): string {
  const rows = data.items
    .map(
      (a) =>
        `<tr>
          <td>${esc(a.activityName)}</td>
          <td>${esc(a.roomName)}</td>
          <td>${esc(formatDateTimeLocal(a.startTime))}</td>
          <td>${esc(formatDateTimeLocal(a.endTime))}</td>
          <td style="text-align:center;font-weight:500">${a.attendanceCount}</td>
        </tr>`,
    )
    .join("");
  ...
  return buildReportPage(
    ...
    data.totalItems,
    ["Actividad", "Sala", "Inicio", "Fin", "Asistentes"],
    rows,
  );
}
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const data = await activeConference.getAttendanceByActivityReport(token, params);
