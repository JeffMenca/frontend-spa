...
import type { ParticipantReportData } from "@/lib/validators/reports";
...
function buildHtml(data: ParticipantReportData, congressId: string): string {
  const rows = data.items
    .map(
      (p) =>
        `<tr>
          <td>${esc(p.personalId)}</td>
          <td>${esc(p.fullName)}</td>
          <td>${esc(p.organization)}</td>
          <td>${esc(p.email)}</td>
          <td>${esc(p.phone)}</td>
          <td>${p.participationTypes.map((t) => esc(t)).join(", ")}</td>
        </tr>`,
    )
    .join("");

  return buildReportPage("Listado de Participantes", `Congreso: ${esc(congressId)}`, data.totalItems, [
    "ID Personal",
    "Nombre completo",
    "Organizacion",
    "Correo",
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const data = await activeConference.getParticipantReport(token, params);
    const congressId = params.get("congressId") ?? "";
    const html = buildHtml(data, congressId);
...
// ──────────────────────────────────────────────────────────────
// Shared HTML template (inline in this module — server-only)
// ──────────────────────────────────────────────────────────────
