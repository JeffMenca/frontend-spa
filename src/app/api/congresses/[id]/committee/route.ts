...
export async function GET(
  ...
  try {
    // TODO(backend-swap): conference GET /congresses/{id}/committee (port 8082)
...
export async function POST(
  ...
  try {
    ...
    // TODO(backend-swap): conference POST /congresses/{id}/committee (port 8082)
    return NextResponse.json(await activeConference.addCommitteeMember(id, body, token), { status: 201 });
  } catch {
    return internalErrorResponse();
  }
}
